...
@Component({
  selector: 'app-vi-tri-nhan-su-edit',
  templateUrl: './vi-tri-nhan-su-edit.component.html',
  styleUrls: ['./vi-tri-nhan-su-edit.component.scss']
})
export class ViTriNhanSuEditComponent extends LayoutComponentBase {
  ...
  InputMaster: PositionModel = new PositionModel();

  @Input() filterInput: PositionModel | undefined = undefined

  @Output() onEditSuccess: EventEmitter<{ code: Action_Type_Enum, data: PositionModel }> = new EventEmitter();
  ...
  setShowPopup(ev: { state: boolean, data: any, typeAction?: Action_Type_Enum }): void {
    ...
    switch (ev.typeAction) {
      case Action_Type_Enum.ADD: {
        this.InputMaster = new PositionModel();
      ...
      case Action_Type_Enum.VIEW: {
        this._loading = true;
        this.hRMCommonClient.positionGet(new PositionModel({
          id: ev.data?.id,
  ...
  UpdateInputMaster() {
    this._loading = true;
    this.hRMCommonClient.positionUpdate(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
  ...
  InsertInputMaster() {
    ...
    this.hRMCommonClient.positionInsert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.showMessageSuccess(res.msg);
        this.onEditSuccess.emit({ code: Action_Type_Enum.SAVE, data: res.data! });
        this.onActionClick_PopupComponent({ code: Action_Type_Enum.CLOSE })
