...
import { HRMCommonClient, PositionModel } from 'src/app/system/server/api_share';
import { ViTriNhanSuEditComponent } from '../vi-tri-nhan-su-edit/vi-tri-nhan-su-edit.component';

@Component({
  selector: 'app-vi-tri-nhan-su-list',
...
export class ViTriNhanSuListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  value: string = '';
  InputMaster: Array<PositionModel> = [];
  filterInput: PositionModel = {} as PositionModel;

  @ViewChild('dataGridComponent') dataGridComponent: DataGridComponent | undefined;
  @ViewChild('viTriNhanSuEditComponent') viTriNhanSuEditComponent: ViTriNhanSuEditComponent | undefined;
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      case Action_Type_Enum.ADD: {
        ...
        if (this.viTriNhanSuEditComponent) {
          this.viTriNhanSuEditComponent.setShowPopup({ state: true, data: null, typeAction: ev.code });
        }
        break;
      }
      ...
      case Action_Type_Enum.COPY: {
        ...
        if (dataSelected && dataSelected[0]) {
          if (this.viTriNhanSuEditComponent) {
            this.viTriNhanSuEditComponent.setShowPopup({ state: true, data: dataSelected[0], typeAction: ev.code });
          }
        } else {
      ...
      case Action_Type_Enum.DELETE: {
        ...
        if (dataSelected && dataSelected[0] && userConfirm) {
          const itemSelected = dataSelected[0] as PositionModel;
          const obj = new PositionModel();
          obj.id = itemSelected.id;
          obj.company_code = itemSelected.company_code;
          this.hRMCommonClient.positionDelete(obj).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this._loading = true;
    this.hRMCommonClient.positionSearch(new PositionModel({
      is_delete: false,
    })).subscribe(res => {
      if (res.status == 0) {
  ...
  ActionEditSuccess(event: { code: Action_Type_Enum, data: PositionModel }) {
