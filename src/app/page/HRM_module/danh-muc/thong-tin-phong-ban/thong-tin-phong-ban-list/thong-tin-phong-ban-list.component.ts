...
import { ThongTinPhongBanEditComponent } from '../thong-tin-phong-ban-edit/thong-tin-phong-ban-edit.component';

@Component({
  selector: 'app-thong-tin-phong-ban-list',
  templateUrl: './thong-tin-phong-ban-list.component.html',
  styleUrls: ['./thong-tin-phong-ban-list.component.scss']
})
export class ThongTinPhongBanListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  value: string = '';
  InputMaster: Array<DepartmentModel> = [];
  filterInput: DepartmentModel = {} as DepartmentModel;

  @ViewChild('dataGridComponent') dataGridComponent: DataGridComponent | undefined;
  @ViewChild('thongTinPhongBanEditComponent') thongTinPhongBanEditComponent: ThongTinPhongBanEditComponent | undefined;
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      case Action_Type_Enum.ADD: {
        ...
        if (this.thongTinPhongBanEditComponent) {
          this.thongTinPhongBanEditComponent.setShowPopup({ state: true, data: null, typeAction: ev.code });
        }
        break;
      }
      ...
      case Action_Type_Enum.COPY: {
        ...
        if (dataSelected && dataSelected[0]) {
          if (this.thongTinPhongBanEditComponent) {
            this.thongTinPhongBanEditComponent.setShowPopup({ state: true, data: dataSelected[0], typeAction: ev.code });
          }
        } else {
      ...
      case Action_Type_Enum.DELETE: {
        ...
        if (dataSelected && dataSelected[0] && userConfirm) {
          const itemSelected = dataSelected[0] as DepartmentModel;
          const obj = new DepartmentModel();
          obj.id = itemSelected.id;
          obj.company_code = itemSelected.company_code;
          this.hRMCommonClient.departmentDelete(obj).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  ActionEditSuccess(event: { code: Action_Type_Enum, data: DepartmentModel }) {
