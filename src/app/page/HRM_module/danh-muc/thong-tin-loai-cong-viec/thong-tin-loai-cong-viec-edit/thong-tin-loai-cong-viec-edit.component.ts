...
import { HRMCommonClient, TypeWorkModel } from 'src/app/system/server/api_share';

@Component({
  selector: 'app-thong-tin-loai-cong-viec-edit',
  templateUrl: './thong-tin-loai-cong-viec-edit.component.html',
  styleUrls: ['./thong-tin-loai-cong-viec-edit.component.scss']
})
export class ThongTinLoaiCongViecEditComponent   extends LayoutComponentBase {
  ...
  InputMaster: TypeWorkModel = new TypeWorkModel();

  @Input() filterInput: TypeWorkModel | undefined = undefined

  @Output() onEditSuccess: EventEmitter<{ code: Action_Type_Enum, data: TypeWorkModel }> = new EventEmitter();
  ...
  setShowPopup(ev: { state: boolean, data: any, typeAction?: Action_Type_Enum }): void {
    ...
    switch (ev.typeAction) {
      case Action_Type_Enum.ADD: {
        this.InputMaster = new TypeWorkModel();
      ...
      case Action_Type_Enum.VIEW: {
        this._loading = true;
        this.hRMCommonClient.typeWorkGet(new TypeWorkModel({
          id: ev.data?.id,
  ...
  UpdateInputMaster() {
    this._loading = true;
    this.hRMCommonClient.typeWorkUpdate(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
  ...
  InsertInputMaster() {
    ...
    this.hRMCommonClient.typeWorkInsert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.showMessageSuccess(res.msg);
        this.onEditSuccess.emit({ code: Action_Type_Enum.SAVE, data: res.data! });
        this.onActionClick_PopupComponent({ code: Action_Type_Enum.CLOSE })
