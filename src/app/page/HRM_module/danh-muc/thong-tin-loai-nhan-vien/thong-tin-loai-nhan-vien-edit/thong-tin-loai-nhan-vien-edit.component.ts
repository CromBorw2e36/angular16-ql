...
@Component({
  selector: 'app-thong-tin-loai-nhan-vien-edit',
  templateUrl: './thong-tin-loai-nhan-vien-edit.component.html',
  styleUrls: ['./thong-tin-loai-nhan-vien-edit.component.scss']
})
export class ThongTinLoaiNhanVienEditComponent  extends LayoutComponentBase {
  ...
  InputMaster: TypeEmployeeModel = new TypeEmployeeModel();

  @Input() filterInput: TypeEmployeeModel | undefined = undefined

  @Output() onEditSuccess: EventEmitter<{ code: Action_Type_Enum, data: TypeEmployeeModel }> = new EventEmitter();
  ...
  setShowPopup(ev: { state: boolean, data: any, typeAction?: Action_Type_Enum }): void {
    ...
    switch (ev.typeAction) {
      case Action_Type_Enum.ADD: {
        this.InputMaster = new TypeEmployeeModel();
      ...
      case Action_Type_Enum.VIEW: {
        this._loading = true;
        this.hRMCommonClient.typeEmployeeGet(new TypeEmployeeModel({
          id: ev.data?.id,
  ...
  UpdateInputMaster() {
    this._loading = true;
    this.hRMCommonClient.typeEmployeeUpdate(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
  ...
  InsertInputMaster() {
    ...
    this.hRMCommonClient.typeEmployeeInsert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.showMessageSuccess(res.msg);
        this.onEditSuccess.emit({ code: Action_Type_Enum.SAVE, data: res.data! });
        this.onActionClick_PopupComponent({ code: Action_Type_Enum.CLOSE })
