...
export class CompanyListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  @ViewChild('companyEditComponent') companyEditComponent: CompanyEditComponent | undefined;
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      case Action_Type_Enum.ADD: {
        ...
        if (this.companyEditComponent) {
          this.companyEditComponent.setShowPopup({ state: true, data: null, typeAction: ev.code });
        }
        break;
      }
      ...
      case Action_Type_Enum.COPY: {
        ...
        if (dataSelected && dataSelected[0]) {
          if (this.companyEditComponent) {
            this.companyEditComponent.setShowPopup({ state: true, data: dataSelected[0], typeAction: ev.code });
          }
        } else {
