...
import { CompanyListManagermentEditComponent } from '../company-list-managerment-edit/company-list-managerment-edit.component';
import { Action_Type_Enum } from 'src/app/components/js-devextreme/popup/enum_action';

@Component({
  selector: 'app-company-list-managerment-list',
  templateUrl: './company-list-managerment-list.component.html',
  styleUrls: ['./company-list-managerment-list.component.scss']
})
export class CompanyListManagermentListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  value: string = '';
  InputMaster: Array<Company> = [];
  filterInput: Company = {} as Company;

  @ViewChild('dataGridComponent') dataGridComponent: DataGridComponent | undefined;
  @ViewChild('companyListManagermentEditComponent') companyListManagermentEditComponent: CompanyListManagermentEditComponent | undefined;
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      case Action_Type_Enum.ADD: {
        ...
        if (this.companyListManagermentEditComponent) {
          this.companyListManagermentEditComponent.setShowPopup({ state: true, data: null, typeAction: ev.code });
        }
        break;
      }
      ...
      case Action_Type_Enum.COPY: {
        ...
        if (dataSelected && dataSelected[0]) {
          if (this.companyListManagermentEditComponent) {
            this.companyListManagermentEditComponent.setShowPopup({ state: true, data: dataSelected[0], typeAction: ev.code });
          }
        } else {
      ...
      case Action_Type_Enum.DELETE: {
        ...
        if (dataSelected && dataSelected[0] && userConfirm) {
          const itemSelected = dataSelected[0] as Company;
          const obj = new Company();
          obj.id = itemSelected.id;
          this.companyClient.delete(obj).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this._loading = true;
    this.companyClient.search(new Company({
      is_delete: false,
    })).subscribe(res => {
      if (res.status == 0) {
  ...
  ActionEditSuccess(event: { code: Action_Type_Enum, data: Company }) {
