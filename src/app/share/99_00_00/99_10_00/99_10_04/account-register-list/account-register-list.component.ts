...
@Component({
  selector: 'app-account-register-list',
  templateUrl: './account-register-list.component.html',
  styleUrls: ['./account-register-list.component.scss']
})
export class AccountRegisterListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      ...
      case Action_Type_Enum.DELETE: {
        const dataSelected = this.dataGridComponent?.getRowSelectedData();
        if (dataSelected && dataSelected[0]) {
          const itemSelected = dataSelected[0] as UserInfo;
          const obj = new Account();
          obj.account = itemSelected.id;
          obj.companyCode = itemSelected.codeCompany;
          this.accountsClient.accountDelete(obj).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
      ...
      case Action_Type_Enum.APPROVAL: {
        ...
        if (dataSelected && dataSelected[0]) {
          ...
          obj.account = itemSelected.id;
          obj.companyCode = itemSelected.codeCompany;
          this.accountsClient.accountChangeStatusApproval(obj).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
              this.showMessageSuccess(res.msg!)
            } else {
  ...
  onLoadData() {
    this.accountsClient.getListUserRegister(new Account()).subscribe(res => {
      if (res.status == 0) {
