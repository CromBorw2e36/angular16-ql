...
import { Account, AccountClientProfileModel, AccountsClient, UserInfo, UserInfoGetListModel } from 'src/app/system/server/api_share';
...
export class AccountListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      case Action_Type_Enum.ADD: {
        this.setRouter(this._urlVoucherFormEdit2[1], null, ev.code);
        break;
      }
      ...
      case Action_Type_Enum.COPY: {
        ...
        if (dataSelected && dataSelected[0]) {
          this.setRouter(this._urlVoucherFormEdit2[0], dataSelected[0], ev.code);
        } else {
