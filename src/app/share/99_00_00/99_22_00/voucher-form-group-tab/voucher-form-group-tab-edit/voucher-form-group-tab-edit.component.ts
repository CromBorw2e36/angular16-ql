...
import { SysVoucherFormClient, SysVoucherFormGroup } from 'src/app/system/server/api_share';

@Component({
  selector: 'app-voucher-form-group-tab-edit',
  templateUrl: './voucher-form-group-tab-edit.component.html',
  styleUrls: ['./voucher-form-group-tab-edit.component.scss']
})
export class VoucherFormGroupTabEditComponent extends LayoutComponentBase implements OnInit, IVoucher_Form_UI, IToolBarComponent {
  ...
  InputMaster: SysVoucherFormGroup = new SysVoucherFormGroup();
  action_type: Action_Type_Enum | undefined

  @Input() dataFilter: SysVoucherFormGroup = new SysVoucherFormGroup();
  ...
  onInitActionAdd() {
    this.InputMaster = new SysVoucherFormGroup();
  }
  ...
  onLoadData(model: SysVoucherFormGroup) {
    this.sysVoucherFormClient.voucherFormGroupSearch(model).subscribe(res => {
      if (res.status == 0) {
  ...
  UpdateData() {
    this.sysVoucherFormClient.voucherFormGroupUpdate(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.showMessageInfo(res.msg!);
        this.location.back();
      } else {
  ...
  InsertData() {
    this.sysVoucherFormClient.voucherFormGroupInsert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.location.back();
