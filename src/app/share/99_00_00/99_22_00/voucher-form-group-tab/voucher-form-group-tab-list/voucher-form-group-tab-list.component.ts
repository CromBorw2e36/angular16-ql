...
import { SysVoucherFormClient, SysVoucherFormGroup } from './../../../../../system/server/api_share';
...
@Component({
  selector: 'voucher-form-group-tab-list',
  templateUrl: './voucher-form-group-tab-list.component.html',
  styleUrls: ['./voucher-form-group-tab-list.component.scss']
})
export class VoucherFormGroupTabListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  value: string = '';
  InputMaster: Array<SysVoucherFormGroup> = [];
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      ...
      case Action_Type_Enum.DELETE: {
        const dataSelected = this.dataGridComponent?.getRowSelectedData();
        if (dataSelected && dataSelected[0]) {
          this.sysVoucherFormClient.voucherFormGroupDelete(dataSelected[0] as SysVoucherFormGroup).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this.sysVoucherFormClient.voucherFormGroupSearch(new SysVoucherFormGroup()).subscribe(res => {
      if (res.status == 0) {
