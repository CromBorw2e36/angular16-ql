import { CategoryCommonClient, CommonContronllerClient, SysPermission } from 'src/app/system/server/api_share';
...
export class PermissionListComponent  extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      ...
      case Action_Type_Enum.DELETE: {
        const dataSelected = this.dataGridComponent?.getRowSelectedData();
        if (dataSelected && dataSelected[0]) {
          this.categoryCommonClient.sysPermissionDelete(dataSelected[0] as SysPermission).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this.categoryCommonClient.sysPermissionSearch(new SysPermission()).subscribe(res => {
      if (res.status == 0) {
