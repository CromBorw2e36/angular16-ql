...
import { CategoryCommonClient, CategoryCommonModel, SysPermission } from 'src/app/system/server/api_share';
...
export class PermissionEditComponent  extends LayoutComponentBase implements OnInit, IVoucher_Form_UI, IToolBarComponent {
  ...
  InputMaster: SysPermission = new SysPermission();
  action_type: Action_Type_Enum | undefined

  @Input() dataFilter: SysPermission = new SysPermission();

  @ViewChild('voucherFormComponent') voucherFormComponent: VoucherFormComponent | undefined;

  ngOnInit(): void {
    const state = this.getRouterState<SysPermission>();
    // console.log(state)
  ...
  onInitActionAdd() {
    this.InputMaster = new SysPermission();
  }
  ...
  onLoadData(model: SysPermission) {
      ...
      if (res.status == 0) {
        if (res.data && res.data.length > 0) {
          ...
          if (this.action_type == Action_Type_Enum.COPY) {
            this.InputMaster.order_number = (this.InputMaster.order_number ?? 0) + 5;
          }
        }
        else
  ...
  InsertData() {
    this.categoryCommonClient.sysPermissionInsert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.location.back();
