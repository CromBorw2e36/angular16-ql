...
@Component({
  selector: 'app-national-and-language-edit',
  templateUrl: './national-and-language-edit.component.html',
  styleUrls: ['./national-and-language-edit.component.scss']
})
export class NationalAndLanguageEditComponent  extends LayoutComponentBase implements OnInit, IVoucher_Form_UI, IToolBarComponent {
  ...
  UpdateData() {
    this.categoryCommonClient.nationalLanguageUpdate(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.showMessageInfo(res.msg!);
        this.location.back();
      } else {
  ...
  InsertData() {
    this.categoryCommonClient.nationalLanguageInsert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.location.back();
