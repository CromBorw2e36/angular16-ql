...
@Component({
  selector: 'app-type-account-list',
  templateUrl: './type-account-list.component.html',
  styleUrls: ['./type-account-list.component.scss']
})
export class TypeAccountListComponent   extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      ...
      case Action_Type_Enum.DELETE: {
        const dataSelected = this.dataGridComponent?.getRowSelectedData();
        if (dataSelected && dataSelected[0]) {
          this.categoryCommonClient.sysTypeAccountDelete(dataSelected[0] as SysTypeAccount).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this.categoryCommonClient.sysTypeAccountSearch(new SysTypeAccount()).subscribe(res => {
      if (res.status == 0) {
