...
export class StatusListComponent  extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      ...
      case Action_Type_Enum.DELETE: {
        const dataSelected = this.dataGridComponent?.getRowSelectedData();
        if (dataSelected && dataSelected[0]) {
          this.categoryCommonClient.sysStatusDelete(dataSelected[0] as SysStatus).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this.categoryCommonClient.sysStatusSearch(new SysStatus()).subscribe(res => {
      if (res.status == 0) {
