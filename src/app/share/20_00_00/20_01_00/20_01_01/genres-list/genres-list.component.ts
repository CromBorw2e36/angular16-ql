...
export class GenresListComponent extends LayoutComponentBase implements OnInit, IDataGridComponent, IToolBarComponent {
  ...
  handleActionClick(ev: I_ToolbarComponent_ActionClick) {
    switch (ev.code) {
      ...
      case Action_Type_Enum.DELETE: {
        const dataSelected = this.dataGridComponent?.getRowSelectedData();
        if (dataSelected && dataSelected[0]) {
          this.movieGenresClient.delete(dataSelected[0] as MovieGenresModel).subscribe(res => {
            if (res.status == 0) {
              this.onLoadData();
  ...
  onLoadData() {
    this.movieGenresClient.search(new MovieGenresModel()).subscribe(res => {
      if (res.status == 0) {
