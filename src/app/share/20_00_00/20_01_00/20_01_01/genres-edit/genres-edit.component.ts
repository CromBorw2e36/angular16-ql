...
import { MovieGenresClient, MovieGenresModel } from 'src/app/system/server/api_share';
...
export class GenresEditComponent extends LayoutComponentBase implements OnInit, IVoucher_Form_UI, IToolBarComponent {

  constructor(
    injector: Injector,
    private movieGenresClient: MovieGenresClient,
  ...
  InputMaster: MovieGenresModel = new MovieGenresModel();
  action_type: Action_Type_Enum | undefined

  @Input() dataFilter: MovieGenresModel = new MovieGenresModel();

  @ViewChild('voucherFormComponent') voucherFormComponent: VoucherFormComponent | undefined;

  ngOnInit(): void {
    const state = this.getRouterState<MovieGenresModel>();
    //console.log(state)
  ...
  onInitActionAdd() {
    this.InputMaster = new MovieGenresModel();
  }
  ...
  onLoadData(model: MovieGenresModel) {
    this.movieGenresClient.search(model).subscribe(res => {
      if (res.status == 0) {
  ...
  UpdateData() {
    this.movieGenresClient.update(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.showMessageInfo(res.msg!);
        this.location.back();
      } else {
  ...
  InsertData() {
    this.movieGenresClient.insert(this.InputMaster).subscribe(res => {
      if (res.status == 0) {
        this.location.back();
